...
def getmention():
	mentions = api.mentions_timeline(q='@nyet_a_russian', since_id=1134003198581190656)
	mention = mentions[0]
	mention_text = mention.text
	mention_id = mention.id
	mention_author = mention.user.screen_name
	return mention_text, mention_id, mention_author
...
previous_mention_id=0

while True:
	mention_text, mention_id, mention_author = getmention() # get the command mention from the authenticated account
	if mention_author not in allowed_list:
		print('Non-authentic user')
		print('Sleeping', sleeptime, 'seconds...')
		sleep(sleeptime)
		continue

	if mention_id == previous_mention_id:
		print('Sleeping', sleeptime, 'seconds...')
		sleep(sleeptime)
		continue
	elif previous_mention_id==0:
		print('Startup')
		previous_mention_id=mention_id
		continue
	else:
		previous_mention_id=mention_id
	
	order = re.search('Command_report(?=:".+")|Command_report(?=:”.+”)',mention_text).group() # Get the order
	command = re.search('(?<=Command_report:").+(?=")',mention_text).group() # Get the topic

	if order=='Command_report':
		print('Operation requested:',order)
		print('Topic requrested:',command)
		query = command

		account, corpus = searchtwit(query, max_tweets)

		#Create edge list
		sender, receiver = edgelister(account, corpus, selfloop, removedups, outputpath) # Call the edgelister function to convert the authors and tweets into an edge list
		...
		#
		influencers_list_str = str(influencers_list)
		influencers_list_str = influencers_list_str.replace('[','')
		influencers_list_str = influencers_list_str.replace(']','')
		influencers_list_str = influencers_list_str.replace("'",'')
		reply_text = '@' + mention_author + ' The most influential users in the network for the topic "' + command + '" are: ' + influencers_list_str
		if len(reply_text) >=280:
			reply_text = reply_text[:277]
			reply_text = reply_text + '...'
		print(reply_text)
		#api.update_status(reply_text, in_reply_to_status_id=mention_id) # disable to just tweet graph
		...
		labeldicto={}
		nx.draw_networkx(network, alpha=0.7, labels=labeldicto, node_color='#23b7ce', node_size=50, font_size=10) # Generate the graph
		#plt.show() # Display the graph
		plt.savefig('C:/Users/Tom Wallace/Dropbox/2PostyGrady_theReturn/Internship/twitter_SNA/temp.png')
		sleep(2)
		graph_reply_text = '@' + mention_author + ' Here is the network graph for the topic: "' + command + '"'
		api.update_with_media('C:/Users/Tom Wallace/Dropbox/2PostyGrady_theReturn/Internship/twitter_SNA/temp.png', status=graph_reply_text, in_reply_to_status_id=mention_id)
		print('End of run, going back to sleep and watching...')
