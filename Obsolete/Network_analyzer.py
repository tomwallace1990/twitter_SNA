...
query = '#DareToCreate' # string. Topic of the request
...
def getusertweets(tweet_author, tweet_text, searched_tweets, oldest_tweet):
	#This function adds the users timeline to the network to increase the chances of finding them, it can also look for mentions of the users to increase the chances further. This biases the network.
	...
	for tweet,author,text in zip(given_user_tweets,given_user_author,given_user_text):
		if query.lower() in text.lower(): # force bother lower case to increase matching
...
def searchtwit(query,max_tweets, allowRTs,get_user_activity):
	#This is the function which makes the API calls for the network. It then does some pre-processing such as getting a list of senders and using the gettext_extended function to get text. It passes back these list as well as the tweet objects in a list as some other functions use them
...
def edgelister(authors, tweets, selfloop=False): 
...
def validate(valid_dict, all_handles):
	...
	for handle in all_handles:
		if handle.lower() in set(validlist):
...
def hasher (sender, receiver, hashing_type, valid_dict, given_user):
	#This function converts non-valid user names to hashes.
...
def edgecounter(senders, receivers, edgeweighting_toggle):
	#This counts the repetitions of dyads to create  edge weights
	#Not sure atm how this works for undirected...

	if edgeweighting_toggle == True:

		dyads = [send+';'+rec for send,rec in zip(senders, receivers)] # create combinations of sender and reciver than can be counted below
...
def centrality(network):
	...
	ds_degree = pd.Series(degree) # networkx returns a dictonary, this is converted into a series with the key becoming the index, when these are combined into a dataframe pandas handles the merge
...
def print_results_userlevel(result_centrality, index_list, random_depth_gain):
...
def simplegraphing(sender,receiver, influencers_list):
	#This produces a simple graph using the inbuilt graphing features of networkx - generally these graphs are poorer than alternatives
...
def htmlgraph(network, sender, receiver, edgeweighting_toggle=False):
	#This function saves data in a format for the R wrapper of the D3 package and then calls a saved R script to generate an HTML file. It then edits a setting in the HTML with regex and opens it. This would be better pipelining into the original D3 in JS.
	...
	unique_set.sort()
	unique_set.append(unique_set.pop(unique_set.index(given_user))) # this ensures the given user is at the end of the list and is given the orange colour by networkD3
	...
	value_exp = edgecounter(largest_comp_send, largest_comp_rec, edgeweighting_toggle) # value by edge weights, from function
	...
	subprocess.call(['C:/Program Files/R/R-3.6.0/bin/Rscript.exe', '--vanilla', './R/GenerateNetwork.R'], shell=False)
...
def sentiment_analysis(corpus,removedups):
	...
	ave_sentiment = sum(sentiment_scores)/float(number_of_tweets) # Calculate the average
	...
	sd = math.sqrt(sum(deviations_square)/number_of_tweets)
	sd = float(str(sd)[:4])
...
if given_user not in all_handles:
